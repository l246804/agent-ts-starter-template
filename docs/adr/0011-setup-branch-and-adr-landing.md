...
- **不覆盖既成文档**：`adr-land` 对落点上已存在的文件拒绝覆盖；brief 按技能自己的规则就地更新，块外内容逐字保留。`docs/agents/*` 是技能自己的**生成物**——技能第 5 步就说重跑是切换 tracker 的方式——所以那三个文件按技能的语义**从种子重新生成**（并显式打印"已存在、已重新生成"），而不是拒绝；"补丁会被静默丢弃"这件事写进陷阱清单（GitHub tracker 那条），而不是变成一个新的拒绝。
...
- **`docs/agents/*` 已存在且内容不同时拒绝** — 拒绝（review 暴露）：那是给技能**发明**一条它没有的规则——技能第 5 步说重跑就是切换 tracker 的方式，它的生成物本就从种子重来。改为"相同则不动、不同则重新生成并打印"，把"手改会被丢弃"这一事实留在陷阱清单里。
- **GitHub 阻塞边陷阱留在 `docs/agent-notes.md` 的基础段里（无条件）** — 拒绝：它只在 setup 真的选了 GitHub tracker 时才成立；改为 `notes-setup`（`when=setup:yes`，且内部再按 `GUIDE_TRACKER=github` 分支），并且**只写进陷阱清单，不修补生成的 `docs/agents/issue-tracker.md`**（约束清单里那条实测结论）。
...
- **两条分支各由 profile 跑一次**：`frontend-single` 取 `yes`（GitHub tracker + `GUIDE_ADR_DIR=docs/decisions`，因此"选是之后落点跟随约定变化"是端到端证据），其余五个 profile 取 `no`。`assert.mjs` 的分支断言全部由**答案**驱动（`GUIDE_SETUP` / `GUIDE_TRACKER` / `GUIDE_DOMAIN_LAYOUT` / `GUIDE_ADR_DIR`），不看目标里恰好有什么。
