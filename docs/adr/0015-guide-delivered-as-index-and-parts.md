...
一个客户端 agent 一次运行只需要本支的步骤，但交付形态此前是整篇 `GUIDE.md`：一次 fetch、263 KB 全进上下文，实际执行的正文只占 44.5–57.8%。本决策把同一个文本按 **`when=` 门**切成 **索引**（头部 + 预答表 + Phase 0–2 + 路由表）与 **分片**（`guide/parts/*.md`）：运行先取索引，再只取自己那一支的分片；`GUIDE.md` 不变，**harness 读取指南的那条路**一行不改（`e2e/extract.mjs` 仍只读 `GUIDE.md`），所以「指南即测试」不重开。harness 此后只改过一处，与读取无关：`neg-verify-red` 控制改成按失败原因判定（`45680f2`）。
...
- 实测（`guide/parts.json` 的字节数，随正文一起增长）：`GUIDE.md` 283.5 KiB；索引 35.7 KiB + 共享分片 `core` 98.1 KiB 是每次运行都读的，分支分片 11–62 KiB 按本支取。**一次运行读 144.9–195.3 KiB（全文的 51–69%）**。
- 与拆分前对比才是这次决策要的数字：那时客户端 agent fetch 整篇（本拆分开工前 263 KiB），现在同样的运行少取 **24–44%**。共享的 98 KiB 是这块收益的上界——其中验证段约 25 KiB（ADR-0006：它只能有一个、不能拆不能门控），其余是每个形状都执行的核心步骤与序言；想再降，得动正文本身（评审里的 pruning 杠杆，未做）。
