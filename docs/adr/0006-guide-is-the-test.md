...
documents. `GUIDE.md` therefore carries machine-extractable step markers — a fenced block
marked `guide:exec` is a shell step to run verbatim, `guide:file` is a document to write
verbatim, `guide:verify` is the assertion set — and `e2e/run.sh` extracts exactly those blocks
and executes them from an empty directory, in document order, stopping at the first failure.
...
- An extracted step that a human cannot paste and run is a bug: the same text serves both
  readers, so steps stay self-contained and start with `set -euo pipefail`.
...
- The harness owns only what the guide does not assert about itself — file tree, document
  placement, the skills lockfile against upstream, and ignore-rule semantics — plus the
  negative controls that prove the checks can fail.
