...
本清单的每条（编号项 `C1..C41`、边界项 `B1..B27`）与仓库里的继承 ADR（`ADR-0001..`）一起构成**条目空间**；每条要么在 `e2e/coverage.mjs` 里有「发货行」（目标项目某文档的某一节、某种形态、一个 marker），要么在该文件的 `NOT_SHIPPED` 里声明不发货的去处（指南步骤 / 验证记录 / 只留本仓库）。两个方向都由 harness 断言：
