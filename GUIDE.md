...
own route plus the built artefact in a backend one.
...
| `fullstack`, and `frontend`/`backend` in any other layout | any | Not implemented yet — the guide stops at the profile guard and tells you so |
...
| --- | --- | --- |
| `GUIDE_MODE` | project mode (形态) | `frontend`, `backend` (this revision) |
| `GUIDE_LAYOUT` | single repo or monorepo | `single` (this revision) |
| `GUIDE_FRAMEWORK` | create-vite template id; in `backend` mode the client is deleted, so only `vanilla-ts` — a base with no framework to unpick — is accepted | `react-ts`, `vue-ts`, `svelte-ts`, `solid-ts`, `preact-ts`, `lit-ts`, `vanilla-ts`, … (frontend); `vanilla-ts` (backend) |
...
| `GUIDE_DEV_PORT` | port the smoke tests use for the dev server | default `5173`; without it Nitro's plugin would pick its own default (`3000`) in `backend` mode |
...
never needs `GUIDE_NITRO_VERSION`.
...
- **Mode** — which kind of project: a pure frontend or a backend. This revision implements those
  two; fullstack (same-origin SSR, or a split frontend/backend) is refused by the guard below
...
- **Framework** — for a frontend, which create-vite TypeScript template; it decides the
  ...
  have to unpick. The guard below refuses anything else before a file is written.
...
case "$GUIDE_MODE/$GUIDE_LAYOUT" in
  ...
  backend/single)
    ...
    ;;
  *)
    printf 'this revision implements only the frontend/single and backend/single profiles; asked for %s/%s.\n' \
...
`nitro.config.ts` have to join one program, and one config extending `nitro/tsconfig` is the
layout that was proven to work — the backend section at the end of this phase is that layout.
...
const keys = source.match(/plugins\s*:/g) ?? [];
if (keys.length !== 1) {
  console.error(`expected exactly one plugins entry in vite.config.ts, found ${keys.length}`);
  process.exit(1);
}
if (!/plugins:\s*\[nitro\(\)\]/.test(source)) {
...
`frontend` mode: in a backend project the server is this project's own, and there is nothing to
proxy to.
...
- **`AGENTS.md`** — what to do. The generator's own marked block stays exactly as written,
  ...
  that mode: a frontend project gets the dev-proxy rules, a backend project the server ones.
...
- Server routes live in `server/routes/`, so their URLs carry no `/api` prefix — that prefix is
  what `server/api/` adds by default, and it belongs to the frontend modes' dev proxy.
...
- The plugin has to be **called** in `plugins`. The scaffold writes no `plugins` key at all, so
  an import without the call is inert: exit 0, and `vp check` reports only
  `Identifier 'nitro' is imported but never used.`
...
- The build script's `tsc` only checks what the tsconfig `include` lists. The config here
  extends `nitro/tsconfig` and lists `server`, `tests` and the config files; without `server`
  in that list, `tsc` passes while never looking at a handler.
...
# frontend project the other way round. Naming the other mode's fact here — a proxy target that
...
verification fails — which is the correct outcome, not a reason to skip the check. The backend
smoke needs nothing external: it starts the artefact the build just produced, then the dev
server, and asserts what each one answers.
...
# to a backend this project does not own, the backend smoke calls a route this project serves.
case "$GUIDE_MODE" in
  frontend)
...
1. **What was built** — the mode, layout, framework (frontend modes) and package manager that
   were chosen, and the versions that actually resolved: `docs/provenance.md` is the record.
2. **What was verified** — the Phase 6 result, and the fact that the type checker was proven
   ...
   built `dist/server/index.mjs` by running it.
3. **What is deliberate** — the pinned prerelease toolchain, and the pinned prerelease Nitro in
   ...
   the backend profile's wired-but-empty `tests/`; and Nitro's output living in `dist/` (backend
   mode), which is why the run added no ignore rule.
4. **What is not covered** — browser hydration is not verified (the smoke test asserts the
   ...
   the project adds later, under the same rules.
...
6. **Commands to live with** — the project's own toolchain, in vp form:
   ...
   move the toolchain forward. In backend mode, `./node_modules/.bin/vp test` runs the suite
